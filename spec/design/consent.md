...
**Spec:** `spec/design/consent.md` · **Version:** 1.19 · **Date:** 2026-09-24 (v1.18: same day · v1.17 · v1.16 · v1.15 · v1.14 · v1.13 · v1.12 · v1.11: 2026-09-23 · v1.10 · v1.9 · v1.8 · v1.7 · v1.6 · v1.5 · v1.4 · v1.3 · v1.2 · v1.1: 2026-09-22 · v1.0: 2026-09-21)
...
**Changelog v1.18 → v1.19 (2026-09-24).** Two minors (a1r, PR #45, review of 2026-09-24 17:04 UTC at `0b40934b`). **The uniform failure page appends on every render** (minor 4): §14.8 (d) covered its pre-trust causes, while R12 `back after decision` and R11 `stale` render the same page and wrote nothing — one render writing once and another not at all. (d) now covers every render, the replay of a decided request included (cause `replayed`); R12's row carries it and R11's points at it. **What bounds the unauthenticated writes** (minor 5): (a), (b) and (d) append from callers who have not signed in; an attempt the throttle refuses is **counted, not appended** — the throttle appends once on engaging and once on releasing, with the refused count — so the chain grows at the throttle's rate, never the caller's. §15 names the surface. D-C1's personal root is written `user:self`, `container.md` §2's one root node.
...
| error (credential) | field cleared; line *That didn't work. Try again.* — identical for wrong, unknown and throttled | ink | `role="status"`; focus to the field | **[GAP→a1p]** §14.8 (a) — one login-attempt append; the cause (wrong · unknown · throttled) in `details`, never on the page (tap spec **D-T8**) |
...
| invalid / expired / used / throttled | field kept; line *That code isn't valid or has expired.*; identical for all four | ink | `role="status"`; focus to the field | **[GAP→a1p]** §14.8 (b) — one device-code redemption append; the cause in `details`, never on the page (**D-T8**) |
...
| stale (request re-submitted after decision: back button) | the uniform failure page — its one append is R12 `back after decision`'s (§14.8 (d)) | — | — |
...
| uniform failure | *This request can't be completed.*; body empty; identical status/length/timing for all pre-trust causes | ink | `<h1>` | **[GAP→a1p]** §14.8 (d) — one append on every pre-trust cause, the cause in `details`, never on the page (**D-T8**) |
...
| back after decision | uniform failure page (the request is single-use) | ink | — | **[GAP→a1p]** §14.8 (d) — one append on the replay of a decided request, cause `replayed`, the cause in `details`, never on the page (**D-T8**) |
