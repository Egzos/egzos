...
**Spec:** `spec/design/step-up-tap-and-pending-approval.md` · **Version:** 1.26 · **Date:** 2026-09-23 (v1.25 · v1.24 · v1.23 · v1.22 · v1.21 · v1.20 · v1.19 · v1.18 · v1.17 · v1.16 · v1.15: same day · v1.14 · v1.13 · v1.12 · v1.11 · v1.10 · v1.9 · v1.8 · v1.7 · v1.6 · v1.5 · v1.4 · v1.3: 2026-09-22 · v1.2: 2026-09-21 · v1.1: 2026-09-13 · v1.0: 2026-09-11)
...
- **D-T9 · `/tap/<token>` takes no parameter; the return address is bound to the token when it is issued, and matched by `consent.md` D-C5.** §2.5 forbade ids in the tap URL while `lifeboat.md` §18 sent `return=/items/<id>` on it, and no rule anywhere validated that `return` — the redirect that fires *after a human has signed inside a presence window*, which is the most valuable phishing target on these pages (a1r major 1, PR #45). a3-trust builds this route at Phase 2.2 from a section that says the URL carries nothing but the token; a5-dinghy would arrive at Phase 4 sending a parameter the route had no reason to accept. Resolved in the shape the flagship already uses (`step-up-integration.md` D-U8, `return_to` in the issuing request's body): **the return address travels with the token, never beside it.** Whoever asks the container for a step-up states where the human goes afterwards — the flagship in `return_to`, the lifeboat's promotion act as the item's own page — and the container validates it **at issue** against D-C5's patterns before binding it; an address that fails binds `/pending`. After a decision the tap page redirects to the bound address; its own affordance stays §13 `tap.return` (*Return to pending*). *Rejected:* allowing a `return` parameter and applying D-C5 to it on the tap page — it keeps an id in the one URL reached by following a token, and it validates at the latest possible moment on the most sensitive page. *Cost:* the step-up-issuing request carries one more field, which §14.3 already asks the contract for.
...
5. **Events — mapped to the drafted taxonomy** (`spec/contracts/events.md`, 2026-09-21). This spec's v1.1 names were freeze inputs; the mapping below is binding for builders, and each **[GAP→a1p]** is raised, not invented:
   ...
   | `tap.redeem_attempt` (R11: unknown / foreign / expired / used token) | **[GAP→a1p]** | **D-T8**: recorded on every path including the successful redemption, exactly one append each so the timing class stays uniform; the cause is the owner's to read in `details` and never reaches the page |
