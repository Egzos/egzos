...
**Spec:** `spec/design/lifeboat.md` · **Version:** 1.21 · **Date:** 2026-09-24 (v1.20: 2026-09-24 · v1.19 · v1.18 · v1.17 · v1.16 · v1.15 · v1.14 · v1.13 · v1.12: 2026-09-23 · v1.11 · v1.10 · v1.9 · v1.8 · v1.7 · v1.6 · v1.5 · v1.4 · v1.3 · v1.2 · v1.1: 2026-09-22 · v1.0: 2026-09-21)
...
**Changelog v1.20 → v1.21 (2026-09-24).** **D-L4 reaches the list, not only the detail** (a1r major 1, PR #45, review of 2026-09-24 17:10 UTC at `0b40934`). v1.20 settled what an item at the personal root renders and landed it in §3.2, §13 and a detail fixture — and nowhere the *row* is described: §2.4 rendered a scope path on every row, R4 had no state for a root item, and §1.1 still read the ring off `visibility.ring`, so the nearest R4 rule (`row · unknown ring`) would have rendered `user` **as a ring word** — the one outcome D-L4 exists to prevent. Now R4 gains `row · personal root`; §2.4 says what the row shows; §1.1 names the **scope** as the discriminator — root-ness is read off the scope's node id, never off `visibility.ring`; §20 gains the Home fixture beside the Detail one. What `visibility.ring` holds for a root item is the one genuinely open question D-L4 creates, and §14.10 had stopped asking — it asks again, `[OPEN→a1p]`, and this spec is built so that no answer changes a render. **`user:ali` → `user:self`** (a1r minor 3): `container.md` §2 names the personal root `self`, so the string a conforming container produces is `user:self`, and a fixture asserting `user:ali` could not pass — three places corrected. (The v1.20 entry below keeps its original wording; it is a record.)

**Changelog v1.19 → v1.20 (2026-09-24).** **The ring vocabulary is `container.md` §1's** (a1r major 2, PR #45): `inbox → thread → project → team → org → exo → uxo → global` — `inbox` added, `enterprise` gone. §14.10 raised the set on the premise that `container.md` *is not drafted yet*; it was, it answers the question, and the false premise left the wrong ring standing. Re-pointed; confirmation only. **D-L4 · the personal root has a scope and no ring** — `container.md` §2 gives it no ring rank — so its detail line reads `in user:ali` from a new key, `item.scope.root`, with the clause absent rather than `ring —`. Fixtures for both.
...
- **D-L4 · An item at the personal root shows its scope and no ring — the clause is absent, not blank.** `container.md` §2 gives the personal root **no ring rank at all** and still puts it in every chain, so `add --scope user:self` produces an item whose detail line has nothing true to say after `· ring`. It reads `in user:self`, from its own key (`item.scope.root`), and nothing else — **in the list row as in the detail** (§2.4, R4 `row · personal root`), and **root-ness is read off the scope's node id (`user:self`), never off `visibility.ring`**, whose value for a root item the contract does not define (§14.10). *Rejected:* `ring —` or `ring none` (a placeholder asserts a ring field that is empty, and *none* reads as a ring named none); `ring personal` (invents a ring word the contract does not have — the one thing this corpus's *render the contract's word verbatim* rule exists to prevent). *Cost:* one more key, and the one place in this spec where a row's shape depends on its scope.
...
- **Scope path** — the node id rendered as `ring:name` (`project:atlas`), with its ring from `visibility.ring`. Rings, innermost first: **inbox → thread → project → team → org → exo → uxo → global** — `container.md` §1's set and rank (drafted; running in `model.RING_RANK`). There is **no `enterprise`** (the contract omits it by decision, v0.5 §A) and `uxo` is held and never instantiated. **The personal root (`user:self`) has no ring rank at all** (`container.md` §2), so an item there has a scope and no ring (**D-L4**) — **and the discriminator is the scope, never `visibility.ring`**: `user` is a root type and not a container type, so the field `context-item.md` §1 describes as *the container type the item currently sits in* has no defined value for a root item (§14.10 asks); a row or a detail whose scope node id is `user:self` renders the scope and no ring word, whatever that field holds. The list still renders an unknown ring name verbatim and never sorts on a hard-coded list.
...
### 2.4 Result row
One `<li>` per item, a link to `/items/<id>`. Anatomy, left to right, wrapping below 700 px: **kind** (mono caps) · **title** (`auto_title`, or `key` if titled empty, else `(untitled)`; one line, ellipsis) · **scope path** (mono, `project:atlas`) · **stamp** (trust word, §7 shape) · **age** (`4 min ago`). Second line (13 px, `--egz-ink-3`): `agent:<actor>` or `<handle>` · `v<version>` · tags as mono words with `#`. Rows carry hairlines only. For an item at the personal root the **scope path** is the root's node id, `user:self`, and no ring word appears anywhere on the row — read off the scope, not `visibility.ring` (R4 `row · personal root`, **D-L4**).
...
### 3.2 Header
Reference line, mono caps: `ITEM 01J7Q4N8 … M3KD · memory · v3`. Title as `<h1>`: the `auto_title` (sentence case as stored; never re-cased), max 2 lines. Under it: `in project:atlas · ring project` — §13 `item.scope`; for an item at the personal root, `in user:self` and **no ring clause at all**, §13 `item.scope.root` (**D-L4**) — (the scope path is a link to `/?q=scope:project:atlas`) · `titled by <engine>` · the trust stamp.
...
| row · personal root | scope path `user:self` (the root's node id); **no ring word on the row** — root-ness is read off the scope, never off `visibility.ring`, so the `unknown ring` rule above does not apply to it (**D-L4**) | ink | — | — |
...
| after post | **303 to the form's own hidden `return` field** — *not* the referrer, which §18 removes: `Referrer-Policy: no-referrer` suppresses the header on the POST too, so a server reading `Referer` here reads nothing. `return` is matched by **`consent.md` D-C5** — the four-step rule that governs `continue`, whose step 3 carries this field's query through so a scheme switch from search results keeps the search; anything else, or nothing, redirects to `/`. The validation is the requirement, not a nicety: an unvalidated `return` on a request that sets a cookie is an open redirect, which is `consent.md` §15's first attack surface. Cookie `egz-scheme` ∈ {light, dark-neutral, dark-violet}; `<html data-scheme data-canvas>` set server-side | — |
...
| item.scope.root | `in <scope>` — the personal root's node id, `user:self`, which has no ring (D-L4) |
...
10. **Ring vocabulary.** **Answered by `container.md` §1** (drafted; in the base since 2026-09-24, and running in `src/egzos/_types.py`): `inbox 0 · thread 1 · project 2 · team 3 · org 4 · exo 5 · uxo 6 · global 7`, fixed by declaration order, **no `enterprise`** (omitted by decision, v0.5 §A), `uxo` held and never instantiated, and **the personal root carrying no ring rank** (§2), which D-L4 answers in this spec. v1.18 raised this as `[OPEN→a1p]` on the stated ground that *`container.md` is not drafted yet* — it was drafted, and the escalation's false premise left a wrong ring in §1.1 while sending a1p to settle something settled (a1r major 2, PR #45). What remains is confirmation only, **[OPEN→a1p]**: the set freezes with `container.md` at 0.3. The UI still renders an unknown ring word verbatim and never sorts on a hard-coded list. **One field stays open, [OPEN→a1p]: what `visibility.ring` holds for an item whose scope is the personal root.** `context-item.md` §1 requires the field and describes it as *the container type the item currently sits in*; the root is a root type with no rank (`container.md` §2), and neither contract says what the field carries there. This spec does not turn on the answer — root-ness is read off the scope (§1.1, D-L4) and `visibility.ring` is never consulted for a root item — so absent, null or a sentinel all render the same; raised so the field is defined before a5-dinghy serialises it (a1r major 1, PR #45).
...
- Two-step act (no JS): the first POST re-renders the page with the button as *Confirm promotion* and a hidden `arm` token valid 10 s; the second POST promotes; a *step-up required* answer redirects (303) to `/tap/<token>` — **no parameter**; the item's own page is bound to the token as its return address when the container issues it, and validated then by `consent.md` D-C5 (tap spec **D-T9**). With htmx: `hx-post` on the form, `hx-target` the acts block, `hx-swap="outerHTML"`.
...
Home: recent ready (12 rows) · recent empty · query ready (counted) · query ready (uncounted, relevance) · no-results · partial (51 rows, Next 50) · invalid cursor · invalid grammar · long title row · unknown ring row · **at the personal root — scope path `user:self`, no ring word on the row** (D-L4) · each kind once (7 rows) · each stamp once (verified, unverified). Detail: text ready · text partial (4 001 chars) · text empty · untitled · artifact inline · artifact image · artifact pdf · artifact other · grant error · oversize · verified (with promotion line) · unverified (with act) · quarantined id (renders the uniform not-found page; asserted identical to the other four causes) · derived_from visible · derived_from not visible · no tags · **at the personal root — `in user:self`, no ring clause, asserted to contain no `ring` word** (D-L4) · **in the inbox — `ring inbox`**. Acts: ready · confirming · in-flight · step-up redirect · promoted · lapsed · invalid (version) · error. Shell: ready · window open · window lapsed · pending 0 / pending 3. Scheme: no cookie · light · dark-neutral · dark-violet · invalid cookie. Global: error card · uniform not-found (**five** causes — not-found, not-yours, tombstoned, quarantined, malformed id — one fixture each, asserted identical in status, length class and timing class). Print: detail ready. Schemes: every fixture in light, dark-neutral, dark-violet.

## 21. Non-goals and open items
Moves, quarantine lifting, token management, audit view (CLI in v0.1) · filters as controls (query prefixes only) · saved searches · the onion · a conversational surface (not in the decisions log) · localisation beyond en-US · **[OPEN→a1p]** the find grammar (§14.1) and the lifeboat's pre-AS login (§14.6) · **[OPEN→CHIEF]** veto window on D-L1–D-L4 before commit.
