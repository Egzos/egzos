...
**Spec:** `spec/design/lifeboat.md` · **Version:** 1.21 · **Date:** 2026-09-24 (v1.20: same day · v1.19 · v1.18 · v1.17 · v1.16 · v1.15 · v1.14 · v1.13 · v1.12: 2026-09-23 · v1.11 · v1.10 · v1.9 · v1.8 · v1.7 · v1.6 · v1.5 · v1.4 · v1.3 · v1.2 · v1.1: 2026-09-22 · v1.0: 2026-09-21)
...
**Changelog v1.20 → v1.21 (2026-09-24).** **D-L4 reaches the list** (a1r, PR #45, review of 2026-09-24 17:04 UTC at `0b40934b`, major 1). It landed in the detail header and not in §2.4's result row, where R4 had no state for a personal-root item — so the nearest rule, `row · unknown ring`, would have rendered `user` **as a ring word**, the outcome D-L4 exists to prevent. R4 gains `row · personal root`; §2.4's scope path names `user:self`; **§1.1 names the discriminator — the scope node, never `visibility.ring`**; §20 gains a Home fixture asserting the unknown-ring rule does not match. §14.10 raises the one question D-L4 creates, `[OPEN→a1p]` and non-blocking: what the required `visibility.ring` field holds for a root item, which neither contract states. **The root is `user:self`** (minor 3): `container.md` §2 names one root node, `self`; D-L4, §3.2 and §20 had written `user:ali`, so the detail fixture could not have passed. (The v1.20 entry below keeps its original wording; it is a record.)
...
- **Scope path** — the node id rendered as `ring:name` (`project:atlas`), with its ring from `visibility.ring`. Rings, innermost first: **inbox → thread → project → team → org → exo → uxo → global** — `container.md` §1's set and rank (drafted; running in `model.RING_RANK`). There is **no `enterprise`** (the contract omits it by decision, v0.5 §A) and `uxo` is held and never instantiated. **The personal root (`user:self`) has no ring rank at all** (`container.md` §2), so an item there has a scope and no ring (**D-L4**). **The scope is the discriminator, never `visibility.ring`:** an item is at the personal root when its scope node *is* the root `user:self` — a root type, not a container type — and the UI reads that from the scope and renders no ring for it, whatever `visibility.ring` holds. What that required field holds for a root item is stated by neither contract; §14.10 asks, and nothing here depends on the answer. The list still renders an unknown ring name verbatim and never sorts on a hard-coded list.
...
| row · personal root | the scope path reads `user:self`, verbatim; **it is not a ring word and this is not `row · unknown ring`** — root-ness is read from the scope node, never from `visibility.ring` (D-L4, §1.1). No other difference: kind, title, stamp and age render as on any row | ink | — | — |
...
| item.scope.root | `in <scope>` — the personal root, which has no ring (D-L4) |
...
10. **Ring vocabulary.** **Answered by `container.md` §1** (drafted; in the base since 2026-09-24, and running in `src/egzos/_types.py`): `inbox 0 · thread 1 · project 2 · team 3 · org 4 · exo 5 · uxo 6 · global 7`, fixed by declaration order, **no `enterprise`** (omitted by decision, v0.5 §A), `uxo` held and never instantiated, and **the personal root carrying no ring rank** (§2), which D-L4 answers in this spec. v1.18 raised this as `[OPEN→a1p]` on the stated ground that *`container.md` is not drafted yet* — it was drafted, and the escalation's false premise left a wrong ring in §1.1 while sending a1p to settle something settled (a1r major 2, PR #45). What remains is confirmation only, **[OPEN→a1p]**: the set freezes with `container.md` at 0.3. The UI still renders an unknown ring word verbatim and never sorts on a hard-coded list. **[OPEN→a1p]** — one question D-L4 creates, raised rather than assumed: `context-item.md` §1 makes `visibility {ring}` a required **running** field, *the container type the item currently sits in*, and for an item whose scope is the personal root — a root, with no ring rank — neither contract says what it holds (absent, a sentinel, the nearest container's word). Non-blocking by construction: the UI reads root-ness from the scope and never from this field (§1.1).
