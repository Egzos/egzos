...
| Notification Channel Matrix — **reference only, not installed** | `felipemenezes098/notifications-1`, demo **#22226** · https://21st.dev/@felipemenezes098/components/notifications-1 | per item page — n/a, no code taken | 2026-09-22 | permissions-matrix.md v1.0 §17 — taken: the row × column layout with both headers locked. Refused: every switch. Its cells are controls; ours are `<td>`s that cannot change anything |
| Role Permissions Picker — **reference only** (already declined for the dashboard, same reason) | `cnippet-dev/role-permissions-picker`, demo **#24929** | per item page | 2026-09-22 | permissions-matrix.md v1.0 §17 — taken: the conceptual principals-down, capabilities-across shape. Refused: checkboxes, coloured badges, the whole editing affordance |
| Records Table — **reference only** | `theshanelevine/records-table`, demo **#23604** | per item page | 2026-09-22 | permissions-matrix.md v1.0 §17 — taken: sticky first column, dense row rhythm. Refused: tag chips, row selection, per-row menus, zebra, sort affordances (D-M11: the ladder is the order) |
| Complex Data Table — **reference only** (fallback reference) | `felipemenezes098/table-20`, demo **#22177** | per item page | 2026-09-22 | permissions-matrix.md v1.0 §17 |
...
| Segmented Control (scope selector fallback, short lists) | `ddoemonn/segmented-control`, demo **#23552** | per item page | 2026-09-22 | permissions-matrix.md v1.0 §17 (strip the sliding thumb — a state change is instantaneous) |
...
| Status · diceui | **declined** · https://21st.dev/diceui/status · **#25393** (the dossier recorded #25395 — corrected here) | MIT (Dice UI) | 2026-09-22 | step-up-integration.md v1.0 §17 — proposed as a structural reference for the chip; it is a pill-shaped badge with a pulsing dot and colour variants, so radius, indicator and palette are all refused and nothing structural remains |
...
## Corrections log
- 2026-09-22 · `Status · diceui` recorded as **#25395** in the 2026-09-21 step-up dossier; the registry item is **#25393** (verified against the catalogue 2026-09-22, before the id reached a spec). The check did more than fix a digit: the component at #25393 is a pill-shaped badge with a pulsing dot and colour variants, so the dossier's description of it as a *horizontal label-and-value container* was wrong too, and the reference is declined rather than re-numbered. Second id this week caught by verifying at the moment of citation (`Empty · cnippet-dev` #19745 → #19746); both would have reached a builder otherwise.
- 2026-09-22 · Two ids the 2026-09-21 `permissions-matrix` dossier carried are wrong and are corrected before first citation, not after: `Data Grid Table · sean0205` is demo **#4821** (dossier: #4783), and the multi-open accordion it cited as **#8637** did not return on the 2026-09-22 pass — `uiable/accordion-multiple` **#29251** is the current item and is what the spec names. A third looked like an error and is not: the dossier's `Table · Origin UI · #99` against this file's **#89** is two demos of one component, exactly what the *an id names a demo* rule describes. Verifying at the moment of citation is what separated the two cases; carrying a dossier id forward is what produced the last three.
