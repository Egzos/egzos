...
- Ownership map (#32): a1p-planner given a writable test path for `src/egzos/_types.py` (`tests/_types/**`, named to avoid colliding with the stdlib `types` module) and for the previously-unowned `tests/test_smoke.py`; `.github/OWNERSHIP.yml` itself added to `chief_only` so a widening of the map is a Watcher-flagged governance-path diff needing the Chief's own merge approval — visibility today, not yet a hard-enforced block, since the ownership check resolves the map from the PR's own head tree (egzos#37 tracks closing that gap); `.claude/agents/a1p-planner.md`'s `Owns` line updated to match
